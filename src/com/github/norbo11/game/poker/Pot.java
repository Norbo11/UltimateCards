...
public class Pot
{
    // This hash map assigns values to every player depending on how much they contributed to this pot.
    private Map<PokerPlayer, Double> contributed = new HashMap<PokerPlayer, Double>();
    private PokerTable pokerTable;                                          // The table which this pot belongs to
    private PokerPlayer playerAllIn;                                        // Holds the player that has created this pot by going all in. This is null if the pot is the main pot
    private ArrayList<PokerPlayer> eligible = new ArrayList<PokerPlayer>(); // This list holds all the players that can currently win this pot
    private double pot;             // The actual amount of the pot
    private boolean main;           // Is true if this pot is the main pot of the table (the first one). This also means playerAllIn is null.
    private int id;                 // The ID of the pot on the table
    private int called;             // Holds the number of people that have called the all-in
    private PokerPhase pokerPhase;  // Represents the phase in which this pot was created
    ...
    public Pot(PokerPlayer playerAllIn, PokerTable pokerTable, double amount, int id)
    {
        ...
        this.main = false;
        this.pokerTable.setLatestPot(this);
    }
    ...
    public void adjustEligible()
    {
        if (!main) 
        {
            if (pokerTable.getCurrentPhase().getNumber() >= PokerPhase.SHOWDOWN.getNumber())
            {
                eligible.clear();
    ...
    public void contribute(PokerPlayer player, double amount, boolean set)
    {
        ...
        {
            //contributed.remove(player);
            contributed.put(player, contributed.get(player) + amount);
        }
    }
    ...
    // Gets the contribution of the specified player. Creates a contribution of 0 if the player doesn't exist
    ...
    {
        double rake = 0;
        if (pokerTable.getSettings().getRake() > 0) // If the table which this pot belongs to has a rake
        {
        ...
        Messages.sendToAllWithinRange(pokerTable.getLocation(), "&6" + playerToPay.getPlayerName() + "&f has won the " + (isMain() ? "main" : "side") + " pot of " + "&6" + Formatter.formatMoney(getPot() - rake));
       
        // Get the actual amount that the player wins by subtracting the rake from the pot, then give it to the player's stack
        playerToPay.giveMoney(getPot() - rake); 
        setPot(0);
    ...
    public String toString()
    {
        ...
        if (!main)
        {
            ...
            {
                eligible = eligible + player.getPlayerName() + ", ";
            }
            
            if (eligible.length() >= 2) eligible = eligible.substring(0, eligible.length() - 2);
        }

        if (main == true) return "&f[Main Pot #" + id + "] " + "&6" + Formatter.formatMoney(pot) + " - For everyone";
        else return "&f[Side Pot #" + id + "] " + "&6" + Formatter.formatMoney(pot) + " - " + eligible;
    }
}
