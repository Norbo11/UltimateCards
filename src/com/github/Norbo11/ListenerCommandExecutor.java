        ...
        if (command.getName().equalsIgnoreCase("hand"))
        {
            if (sender instanceof Player)
            {
                ...
                if (args.length > 0)
                {
                    ...
                    if (action.equalsIgnoreCase("pot"))
                    {
        ...
        if (command.getName().equalsIgnoreCase("table"))
        {
            if (sender instanceof Player)
            {
                ...
                if (args.length > 0)
                {
                    ...
                    {
                        if (args.length == 1) p.methodsTable.displayTables(player);
                        else p.methodsError.usage(player, "list");
                        return true;
                    }
                    if (action.equalsIgnoreCase("create") || action.equalsIgnoreCase("new"))
                    {
                        if (args.length == 2) p.methodsTable.createTable(player, args[1]);
                    ...
                    {
                        if (args.length == 3) p.methodsTable.sitTable(player, args[1], args[2]);
                        else p.methodsError.usage(player, "sit");
                        return true;
                    }
                    if (action.equalsIgnoreCase("getup") || action.equalsIgnoreCase("standup") || action.equalsIgnoreCase("leave"))
                    {
                        if (args.length == 1) p.methodsTable.leaveTable(player);
                        else p.methodsError.usage(player, "getup");
                        return true;
                    }
                    if (action.equalsIgnoreCase("tp"))
                    {
                        if (args.length == 2) p.methodsTable.tpToTable(player, args[1]);
                    ...
                    {
                        if (args.length >= 1 && args.length <= 3) p.methodsTable.listDetails(player, args[1], args[2]);
                        else p.methodsError.usage(player, "details");
                        return true;
                    }
                    if (action.equalsIgnoreCase("board"))
                    {
                        if (args.length == 2) p.methodsTable.displayBoard(player, args[1]);
                        else p.methodsError.usage(player, "board");
                        return true;
                    }
                    ...
                    if (action.equalsIgnoreCase("listplayers"))
                    {
                        if (args.length == 1) p.methodsTable.displayPlayers(player);
